namespace SalesForce.Pages
{
    ...
    public class CaseMassDeletePage : BasePage
    {
        #region Constructor
        public CaseMassDeletePage(IWebDriver Driver) : base(Driver)
        {
        }
        #endregion
        ...
        {
            Assert.AreEqual("Mass Delete: Cases ~ Salesforce - Enterprise Edition", driver.Title);
            Assert.AreEqual("Step 1: Review what will happen when you mass delete your Cases:", HeadStep1Text.Text);
            Assert.AreEqual("Step 2: Recommendation prior to mass deleting:", HeadStep2Text.Text);
            Assert.AreEqual("Step 3: Find Cases that match the following criteria:", HeadStep3Text.Text);
